...
void janus_ice_cb_component_state_changed(NiceAgent *agent, guint stream_id, guint component_id, guint state, gpointer ice)
{
  ...
  if ((state == NICE_COMPONENT_STATE_CONNECTED || state == NICE_COMPONENT_STATE_READY) && handle->send_thread == NULL)
  {
...
void janus_ice_cb_nice_recv(NiceAgent *agent, guint stream_id, guint component_id, guint len, gchar *buf, gpointer ice)
{
  ...
  if (component_id == 3 || (janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_RTCPMUX) && janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_DATA_CHANNELS)))
  {
...
static gint janus_pkt_queue_depth_ms(janus_ice_handle *handle, gboolean video_pkt_queue, rtp_header *header)
{
...
void janus_ice_relay_rtp(janus_ice_handle *handle, int video, char *buf, int len)
{
  ...
  {
    JANUS_LOG(LOG_VERB, "PKT_QUEUE has %d ms in it, discarding packet\n", queue_ms);
    return;
  }
